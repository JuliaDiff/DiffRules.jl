...
macro define_diffrule(def)
    return define_diffrule(__module__, def)
end

function define_diffrule(m::Module, def::Expr)
    @assert(Meta.isexpr(def, :(=)) && Meta.isexpr(def.args[1], :call),
            "diffrule must be written in method definition assignment form")
    lhs = def.args[1]
    rhs = def.args[2]
    qualified_f = lhs.args[1]
    @assert(isa(qualified_f, Expr) && qualified_f.head == :(.),
            "rule name must be fully qualified (e.g. `Base.sin`, not `sin`)")
    M = qualified_f.args[1]
    f = _get_quoted_symbol(qualified_f.args[2])
    args = lhs.args[2:end]
    key = (M, f, length(args))
    DiffRules.DEFINED_DIFFRULES[key] = Core.eval(m, Expr(:->, Expr(:tuple, args...), rhs))
    return key
end
...
Return the derivative expression(s) for `M.f` at the given argument(s), with the argument(s)
...
"""
diffrule(M::Union{Expr,Symbol}, f::Symbol, args...) = Base.invokelatest(DEFINED_DIFFRULES[M,f,length(args)], args...)
